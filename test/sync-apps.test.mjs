...
test("bootstraps a new app from its latest release", async () => {
  archive = await createArchive();
  const catalog = await createCatalog();
  try {
    await runSync(catalog);
    assert.equal(JSON.parse(await readFile(path.join(catalog, ".generated-apps.json"))).apps.example.tag, "v1.0.0");
  } finally {
    await rm(catalog, { recursive: true, force: true });
  }
});
